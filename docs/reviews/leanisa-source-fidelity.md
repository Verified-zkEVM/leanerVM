...
including the whole diff of `cpu/execute.rs`, `cpu/mod.rs`, `cpu/hints.rs`, `cpu/layout.rs`,
`hash_flock.rs`, `flock/src/hash.rs`, `tables.rs`, `constraints.rs`, and the §2 specification.
`cpu/isa.rs`, `crates/primitives/src/hash.rs`, `07-instruction-tables.tex`, and the compression
core at `flock/src/hash.rs:191-231` are unchanged. The comparison pin adds three execution hints
...
bit-encoding explanation. `hash_flock.rs` changes setup caching, while `tables.rs` and
`constraints.rs` change the quadratic-part interface used by sumcheck; the full row constraints
